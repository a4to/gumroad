...
class SuspendAccountsWithPaymentAddressWorker
  ...
  def perform(user_id)
    ...
    if suspended_user.suspended_for_tos_violation?
      probate_users_with_same_payment_address(suspended_user)
      probate_users_with_same_stripe_fingerprint(suspended_user)
    else
  ...
  private
    ...
    def probate_users_with_same_payment_address(suspended_user)
      return if suspended_user.payment_address.blank?
      ...
      User.not_suspended
        ...
        .where.not(id: suspended_user.id)
        .find_each do |user|
          put_user_on_probation(user, suspended_user, "payment address", suspended_user.payment_address)
        end
    end

    def probate_users_with_same_stripe_fingerprint(suspended_user)
      fingerprints = suspended_user.bank_accounts.where.not(stripe_fingerprint: [nil, ""]).distinct.pluck(:stripe_fingerprint)
      return if fingerprints.empty?
      ...
      User.not_suspended.where(id: user_ids_with_same_fingerprint).find_each do |user|
        matching_fingerprint = (fingerprints & user.alive_bank_accounts.pluck(:stripe_fingerprint)).first
        put_user_on_probation(user, suspended_user, "bank account fingerprint", matching_fingerprint)
      end
    end
    ...
    def put_user_on_probation(user, suspended_user, identifier_type, identifier_value)
      return unless user.can_flag_for_fraud?

      user.put_on_probation(
        author_name: "suspend_sellers_other_accounts",
        content: "Probated (payouts suspended) automatically on #{Time.current.to_fs(:formatted_date_full_month)} because this account uses #{identifier_type} #{identifier_value}, which matches User##{suspended_user.id} (UID: #{suspended_user.external_id}) suspended for a policy violation"
      )
    end
end
